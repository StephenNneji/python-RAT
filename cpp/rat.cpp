...
const std::string docs_Control = R"(Control class for the C++ bindings.
...
const std::string docs_RATMain = R"(Entry point for the main reflectivity computation.

Parameters
----------
problem_def : Rat.rat_core.ProblemDefinition
    The problem input for the RAT calculation.
limits : RATapi.rat_core.Limits
    Min and max values for each parameter defined in the probelm definition.
control : RATapi.rat_core.Control
    The controls object for the RAT calculation.
priors : RATapi.rat_core.Priors
     The priors defined for each parameter in the problem definition.

Returns
-------
out_problem_def : Rat.rat_core.ProblemDefinition
    The problem input with the updated fit values.
...
py::tuple RATMain(const ProblemDefinition& problem_def, const Limits& limits, const Control& control, const Priors& priors)
{
    ...
    RAT::struct9_T bayes_results;
    // Call the entry-point
    RAT::RATMain(&problem_def_struct, &limits_struct, &control_struct, &priors_struct, &results, &bayes_results);
    ...
    return py::make_tuple(out_problem_def, 
                          ...
                          bayesResultsFromStruct9T(bayes_results));    
}

const std::string docs_makeSLDProfileXY = R"(Creates the profiles for the SLD plots
...
PYBIND11_MODULE(rat_core, m) {
    ...
    py::class_<Control>(m, "Control", docs_Control.c_str())
        .def(py::init<>())
    ...
    m.def("RATMain", &RATMain,  docs_RATMain.c_str(),
          py::arg("problem_def"), py::arg("limits"), py::arg("control"), py::arg("priors"));

    m.def("makeSLDProfileXY", &makeSLDProfileXY, docs_makeSLDProfileXY.c_str(), 
